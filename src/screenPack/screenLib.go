...
func ProveriPritisakNaGumb(matrix [][]mat.Cestica, x, y int32) {
	...
	if x > SirinaProzora-MarginaZaGumbad+SirinaUIMargine && x < SirinaProzora-SirinaUIMargine {
		...
		// FPS LIMIT
		if y > VisinaProzora-5*(VisinaDugmeta+VisinaUIMargine) && y < VisinaProzora-5*(VisinaDugmeta+VisinaUIMargine)+VisinaDugmeta {
			matrixPack.FpsCap = (matrixPack.FpsCap * 2) % 210
		}
		...
		if y > VisinaProzora-2*(VisinaDugmeta+VisinaUIMargine) && y < VisinaProzora-2*(VisinaDugmeta+VisinaUIMargine)+VisinaDugmeta {
			...
			sdl.ShowSimpleMessageBox(sdl.MESSAGEBOX_INFORMATION, "Pesak", languagePack.SlikaSeSacuvalaTekst[mat.IzabraniJezik], nil)
		}
